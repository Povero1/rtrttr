...
client.on("ready", () => {
  ...
  client.user.setActivity(`#- ELITE S`, { type: 'WATCHING' })
});
...
client.on('messageCreate', message => {
  ...
  if (message.channel.id == `1236777517877166121`){
...
client.on('messageCreate', async (message) => {
    if (message.content.startsWith(prefix + 'kr') && message.member.roles.cache.some(role => role.name === '♚ || Discord Owners :')) {
        const args = message.content.split(' ');
        const roleName = args[1];
        const roleColor = args[2];
        const mentionedUser = message.mentions.users.first();
        const mentionedMember = message.guild.members.cache.get(mentionedUser.id);
        const roleChannel = message.guild.channels.cache.find(channel => channel.name === 'ℵ・لوج・خاص');
...
client.on('messageCreate', (message) => {
    if (message.content.startsWith( prefix + 'hexcode')) {
        const args = message.content.split(' ');
...
client.on(`messageCreate`, Kimo => {
  ...
  if (command === prefix + `r`) {
    const rolesList = [
      { name: 'ES | SELLER', id: '1236597144010625135' },
      { name: 'ES | MEMBER', id: '1236417630827708416' },
      { name: 'chance s', id: '1240701406823055524' },
      { name: 'soon', id: '' },
      { name: 'soon', id: '' },
      { name: 'soon', id: '' },
      { name: 'soon', id: '' }
    ...
    if (!Kimo.member.roles.cache.has("1238171615997595791"))/* ايدي الرول الادارة اللي تقدر تعطي الرولات*/  return Kimo.reply("ليس لديك صلاحيات لاستخدام الأمر");
    ...
    Kimo.reply(`الرجاء اختيار الرول المطلوب من القائمة التالية:\n${roleOptions}`).then(() => {
      ...
      Kimo.channel.awaitMessages({ filter, max: 1, time: 30000, errors: ['time'] })
        .then(collected => {
...
لا تنسي قرأة القوانين ووضع فيدباك عن : <@${admin.id}>**`);
          ...
          const roleslog = Kimo.guild.channels.cache.get('1242493572364894280');
...
client.on('messageCreate', async (message) => {
  if (message.content.startsWith(prefix + 'cr')) {
    ...
    const allowedRole = message.guild.roles.cache.find(role => role.name === '♔・↝ Discord Leader :');

    if (!message.member.roles.cache.has(allowedRole.id)) {
      message.channel.send('يجب أن تكون من طاقم الادارة العليا!');
      return;
    }
    ...
    const logChannel = message.guild.channels.cache.get('1163720743998197822');
    if (logChannel) {
...
المسؤول : ${admin}.`)
    ...
    const logChannelId = '1163720743998197822';
...
client.on('messageCreate', async (message) => {
  if (message.content.startsWith(prefix + 'come')) {
    if (message.author.bot) return;
    if (!message.member.roles.cache.has(discordStaff)) return; // ايدي الرتبة الي تقدر تستعمل الامر
...
let autotax = ['1239179218072637481'];
...
let autoline_channel = [''] // ايدي الرومات
...
///////////////
client.on('messageCreate', async message => {
  if (message.content.startsWith(prefix + 'تقييم') && message.member.roles.cache.has(discordStaff)) {
message.channel.send(` **كان معاك الاداري** <@${message.author.id}>
 **يرجئ تقييمي هنا** <#1236430557601333270>
 ****فضلاً وليس أمراً  ****`)
...
const message = await interaction.channel.messages.fetch(interaction.message.id);
const now = new Date();
  const ManshoratChannel = client.channels.cache.get('1163720653145395210');
  const ManshoratLog = interaction.guild.channels.cache.get('1163720743998197822');
...
const message = await interaction.channel.messages.fetch(interaction.message.id);
const now = new Date();
  const ManshoratChannel2 = client.channels.cache.get('1163720653145395210');
  const ManshoratLog2 = interaction.guild.channels.cache.get('1163720743998197822');
...
const message = await interaction.channel.messages.fetch(interaction.message.id);
const now = new Date();
  const adChannel = client.channels.cache.get('1163720639442587708');
  const adLog = interaction.guild.channels.cache.get('1163720743998197822');

  const memhere = `${member}`
  const manshorhere = `@here\n${manshor}\nاعلان مدفوع فقط تبي مثله حياك <#1163720697797955674>`
...
const message = await interaction.channel.messages.fetch(interaction.message.id);
const now = new Date();
  const adChannel2 = client.channels.cache.get('1163720639442587708');
  const adLog2 = interaction.guild.channels.cache.get('1163720743998197822');
...
client.on('interactionCreate', async interaction => {
    ...
    if(interaction.isButton){
        ...
        db.add(`alluser_${interaction.member.id}`, 1)
          ...
          const everyone = interaction.guild.roles.cache.find(r => r.name === "ES | NEW STAFF");
...
interaction.channel.setName("ticket-" + interaction.user.username);                interaction.channel.permissionOverwrites.edit(role,{SEND_MESSAGES: true})
          ...
          interaction.channel.permissionOverwrites.edit('1152236106973196370',{SEND_MESSAGES: false})
          interaction.channel.permissionOverwrites.edit('1152236106973196370',{VIEW_CHANNEL: true})
          }else{
            interaction.followUp({ 
                content: `كخ يابابا`,
                ephemeral: true 
...
const warnWork = `1239690685532209322`
client.on("messageCreate" , message => {
  if(message.content.startsWith(prefix + "تحذير")) {
    ...
    let madri = `(=)`
    let channel = client.channels.cache.get('1239690770387177582');
    let warn2 = client.channels.cache.get('1239692220328575036');

    let s1 = message.guild.roles.cache.find(r => r.name == "Warn 50%");
    let s2 = message.guild.roles.cache.find(r => r.name == "Warn 100%");

    let perfect = message.guild.roles.cache.find(r=>r.name == "ES | SELLER")
    let gold = message.guild.roles.cache.find(r=>r.name == "ES | MEMBER")
    ...
    warn2.send({content:`:warning: **تحذير** \n ** الشخص : ${user}\n الإداري : ${message.member}\n نوع التحذير : Warn 50%\n السبب : لمعرفة السبب يرجى يفتح تكت <#1163720697797955674>**`})
      warn2.send(`${lineLink}`);    
      user.roles.add([s1])
    ...
    channel.send({content:`** الشخص : ${user}\n\n الإداري : ${message.member}\n\n نوع التحذير : Warn 100%\n\n السبب : ${reason}\n\n الدليل :**` , files:attachmentFiles})
      channel.send(`${lineLink}`);
    warn2.send({content:`:warning: **تحذير** \n ** الشخص : ${user}\n الإداري : ${message.member}\n نوع التحذير : Warn 100%\n السبب : لمعرفة السبب يرجى يفتح تكت <#1236424661986971668>**`})
      warn2.send(`${lineLink}`);    
      user.roles.add([s2])
    ...
    db.add(`allwarns_${message.member.id}` , 1)
     const attachments = Array.from(message.attachments.values());
      const attachmentFiles = attachments.map((attachment) => attachment.url);
  channel.send({content:`** الشخص : ${user}\n\n الإداري : ${message.member}\n\n نوع التحذير : Warn سحب الرتبة\n\n السبب : ${reason}\n\n الدليل :**` , files:attachmentFiles})
      channel.send(`${lineLink}`);
    warn2.send({content:`:warning: **تحذير** \n ** الشخص : ${user}\n الإداري : ${message.member}\n نوع التحذير : سحب الرتبة\n <#1236424661986971668> السبب : لمعرفة السبب يرجى يفتح تكت **`})
      warn2.send(`${lineLink}`);    
      user.roles.remove([s1,s2,perfect,gold])
        ...
        if (message.member.permissions.has("ADMINISTRATOR")) {
            ...
            const memberList = await message.guild.members.fetch();
              let usersData = [];
                ...
                const channel = message.guild.channels.cache.get('1237487827449679922');
        ...
        try {
            ...
            memberList.forEach(async (member) => {
                if (member.roles.cache.has('1152236106973196370')) {  //discord staff
                    ...
                    var roleToAssignHighStaff = "1158302520490336296"; 
